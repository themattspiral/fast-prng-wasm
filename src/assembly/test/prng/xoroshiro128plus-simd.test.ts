...
describe('Xoroshiro128PlusSIMD', () => {
  ...
  describe('Array Methods', () => {
    ...
    test('uint64Array produces deterministic sequences', () => {
      ...
      expect(mismatchCount).toBe(0); // All array values are deterministic with same seeds
    });
  ...
  describe('SIMD-Specific Tests', () => {
    test('uint64x2 produces independent non-zero values in both lanes', () => {
      ...
      expect(zeroCountLane0).toBe(0); // Lane 0: no zero values
      expect(zeroCountLane1).toBe(0); // Lane 1: no zero values
    });
    ...
    test('interleaved identical streams with duplicated seeds', () => {
      ...
      expect(mismatchCount).toBe(0); // All pairs match when seeds are duplicated
    });
    ...
    test('interleaved matching streams: both lanes match their own non-SIMD sequences', () => {
      ...
      expect(lane0MismatchCount).toBe(0); // All lane 0 values match non-SIMD primary seed sequence
      expect(lane1MismatchCount).toBe(0); // All lane 1 values match non-SIMD alternate seed sequence
    });
  });
});
